...
NEWS_QUERY_TERMS = (
    '(dollar OR USD OR BRL OR "Brazil real" OR Brazil OR Fed OR '
    'inflation OR interest OR rates OR Treasury OR "emerging markets" OR '
    'fiscal OR deficit OR debt OR commodities OR oil OR tariffs OR geopolitics)'
)
...
def headline_relevance(title):
    ...
    if any(k in t for k in ["usd/brl","brl","brazil real"]):
        hits += 3
    if any(k in t for k in ["fed","federal reserve","treasury","inflation","interest rate"]):
        hits += 2
    if hits >= 4:
        return "alta"
    if hits >= 2:
...
def collect_public_news():
    ...
    for source_name, domain in NEWS_SOURCES:
        try:
            by_source[source_name].extend(gdelt_articles(source_name, domain))
        except Exception as e:
    ...
    for source in list(by_source):
        ...
        for x in by_source[source]:
            ...
            seen.add(key)
            dedup.append(x)
        by_source[source] = dedup[:8]

    return by_source, failures
...
def source_summary(source, items):
    if not items:
        return f"{source}: nenhuma manchete relevante encontrada nas últimas 48 horas."
    ...
    if pos > neg and pos >= 2:
        direction = "predomínio de fatores potencialmente altistas para o USD/BRL"
    elif neg > pos and neg >= 2:
        direction = "predomínio de fatores potencialmente baixistas para o USD/BRL"
    else:
        direction = "leitura mista/neutra nas manchetes relevantes"
    return (
        f"{source}: {direction}; {pos} altista(s), {neg} baixista(s) "
        f"e {neu} neutra(s) entre {len(items)} manchete(s) relevante(s)."
    )
...
def build_intelligence(latest):
    ...
    public_news, news_failures = collect_public_news()
    media_score_total = 0
    ...
    summary.append("A leitura é determinística, usa dados/metadados públicos e não utiliza API de IA paga.")

    result = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "method": "Motor determinístico SEMIL V2 — custo zero + notícias públicas via GDELT",
        "score10": score,
        "outlook10": {
            "bias": bias,
